...
function reconnectProvider(_tries){try{summitEthProvider=new Web3.providers.WebsocketProvider(summitServer);summitEthProvider.on('connect',function(){summitEthConn=new Web3(summitEthProvider);summitEthContract=new summitEthConn.eth.Contract(summitAbi.abi,ethContractAddr)});summitEthProvider.on('error',function(e){reconnectProvider(_tries++)});summitEthProvider.on('end',function(e){reconnectProvider(_tries++)})}catch(e){_tries++;if(_tries<20){setTimeout(function(){reconnectProvider(_tries)},500)}else{}}}
function getSummitStats(){;summitEthContract.methods.totalShares().call(function(_err,_result){if(!_err){let _totalEth=new BigNumber(_result);let _btc=_totalEth.div(TOKEN_DECIMALS).div(new BigNumber(exchange_btc_eth));let _usd=_btc.times(exchange_btc_usd);}});setTimeout(function(){getSummitStats()},15000)}
...
setupButtons();function checkNetwork(_callback){web3Provider.eth.net.getNetworkType(function(err,res){ethContract=new web3Provider.eth.Contract(summitAbi.abi,ethContractAddr);if(_callback)
_callback()})}
...
function setupUser(_callback){populateEarnings(1);populateDirectLevels(1)}
function populateDirectLevels(level){ethContract.methods.usersActiveDirectLevels(usrWalletAddress,level).call(function(_err,_result){if(_result==!0){$('#direct_level'+level+'_stats').removeClass('hide');$('#direct_level'+level+'_buy').removeClass('hide');$('#direct_level'+level+'_buy').addClass('hide');$('#direct_level'+level).removeClass('hide');$('#direct_level'+level).removeClass('disabled');ethContract.methods.usersDirectMatrix(usrWalletAddress,level).call(function(_err,_results){ethContract.methods.users(_results[0]).call(function(_err,_result){if(!_err){var t=Number(_result.id);if(t==1&&userId>20)t=Math.round((userId+level)/2+(userId/7)+(level*2));$('#direct_level'+level+'_referrer').html(t+0);$('#direct_level'+level+'_referrer').attr("title","Current Referrer ID:"+(t+0))}});if(_results[2]==!0){$('#direct_level'+level).addClass('disabled');toastMessage("Level "+level+" is blocked for invites until you unlock the next level!","Upgrade needed",30000)}
for(let c=0;c<_results[1].length;c++){ethContract.methods.users(_results[1][c]).call(function(_err,_result){$('#direct_idOfLevel'+level+'_'+Number(c+1)).html(Number(_result.id)+0);$('#direct_idOfLevel'+level+'_'+Number(c+1)).attr("title","Friend ID:"+_result.id+","+" Addr:"+_results[1][c])})}
if(level<13)
...
function populateMultiLevels(level){ethContract.methods.usersActiveMultiLevels(usrWalletAddress,level).call(function(_err,_result){if(_result==!0){$('#multi_level'+level+'_stats').removeClass('hide');$('#multi_level'+level+'_buy').removeClass('hide');$('#multi_level'+level+'_buy').addClass('hide');$('#multi_level'+level).removeClass('disabled');$('#multi_level'+level).removeClass('hide');ethContract.methods.usersMultiMatrix(usrWalletAddress,level).call(function(_err,_results){ethContract.methods.users(_results[0]).call(function(_err,_result){if(!_err){var t=Number(_result.id);if(t==1&&userId>20)t=Math.round((userId+level)/2+(userId/7)+(level*2));$('#multi_level'+level+'_referrer').html(t+0);$('#multi_level'+level+'_referrer').attr("title","Current Referrer ID:"+(t+0))}});if(_results[3]==!0){$('#multi_level'+level).addClass('disabled');toastMessage("Level "+level+" is blocked for invites until you unlock the next level!","Upgrade needed",30000)}
for(let c=0;c<_results[1].length;c++){ethContract.methods.users(_results[1][c]).call(function(_err,_result){$('#multi_idOfLevel'+level+'_1_'+Number(c+1)).html(Number(_result.id)+0);$('#multi_idOfLevel'+level+'_1_'+Number(c+1)).attr("title","Friend ID:"+_result.id+","+" Addr:"+_results[1][c])})}
for(let c=0;c<_results[2].length;c++){ethContract.methods.users(_results[2][c]).call(function(_err,_result){$('#multi_idOfLevel'+level+'_2_'+Number(c+1)).html(Number(_result.id)+0);$('#multi_idOfLevel'+level+'_2_'+Number(c+1)).attr("title","Friend ID:"+_result.id+","+" Addr:"+_results[2][c])})}
if(level<13)
...
var sharesClaimed=new BigNumber(0);var sharesAvail=new BigNumber(0);function showUser(_UserObj){$('#regDiv').addClass('hide');showAccountPage();$('#inpReferrer2').val(_UserObj.referrer);$("#inpUserId").val(Number(_UserObj.id)+0);$("#inpUserAddress").val(usrWalletAddress);sharesClaimed=new BigNumber(_UserObj.sharesClaimed);getShares()}
...
return}
$("#inpReferrer").on("change, input",function(e){if($("#inpReferrer").val()==""){referrer=defAcc;Cookies.set('ref',referrer,{expires:60});$("#btnReg").show();$("#btnVerify").hide()}else{$("#btnReg").hide();$("#btnVerify").show()}});$("#btnVerify").on("click",function(e){var v=$("#inpReferrer").val();if(v==""){referrer=defAcc;Cookies.set('ref',referrer,{expires:60});$("#btnReg").show();$("#btnVerify").hide();return}
showLoader("wait");if(is_url(v)){if(v.indexOf("ref=0x")>=0){v=v.substr(v.indexOf("ref=0x")+4)}else{hideLoader();referrer=defAcc;Cookies.set('ref',referrer,{expires:60});toastMessage("Invalid referrer!")}}
