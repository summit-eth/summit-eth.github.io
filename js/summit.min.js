...
summitServer="wss://mainnet.infura.io/ws/v3/"+summitServer;const isMetaMaskInstalled=()=>{const{ethereum}=window;return Boolean(ethereum&&ethereum.isMetaMask)};const onClickConnect=async()=>{try{showLoader("wallet");$(".landing-header-sub,.landing-summary,#btnViewFaq, #btnViewGuides").hide();if(ethereum.enable){try{await ethereum.enable();userWalletEnabled=!0;beginSetup()}catch(e){}}else if(window.web3){userWalletEnabled=!0;beginSetup()}else{beginSetup()}}catch(error){hideLoader();toastMessage("An error occured","Refreshing Page...",2000);setTimeout(function(){top.location.href=top.location.href},2000)}};setupRates();getGasPrices();var readonlyEnv=!1;setupReadOnlyWeb3(function(){readonlyEnv=!0});function setupReadOnlyWeb3(_callback){summitEthProvider=new Web3.providers.WebsocketProvider(summitServer);summitEthProvider.on('connect',function(e){summitEthConn=new Web3(summitEthProvider);summitEthContract=new summitEthConn.eth.Contract(summitAbi.abi,y(ethContractAddr));getSummitStats();if(_callback)
_callback();setTimeout(function(){},2000)});summitEthProvider.on('error',function(e){toastMessage("Reconnecting to server...","Disconnected",2000);reconnectProvider(0)});summitEthProvider.on('end',function(e){toastMessage("Reconnecting to server...","Disconnected",2000);reconnectProvider(0)})}
function reconnectProvider(_tries){try{summitEthProvider=new Web3.providers.WebsocketProvider(summitServer);summitEthProvider.on('connect',function(){toastMessage("Re-established connection to server","Connected",2000);summitEthConn=new Web3(summitEthProvider);summitEthContract=new summitEthConn.eth.Contract(summitAbi.abi,y(ethContractAddr))});summitEthProvider.on('error',function(e){reconnectProvider(_tries++)});summitEthProvider.on('end',function(e){reconnectProvider(_tries++)})}catch(e){_tries++;if(_tries<20){setTimeout(function(){reconnectProvider(_tries)},500)}else{}}}
function getSummitStats(){;summitEthContract.methods.lastUserId().call(function(e,res){if(!e){$("#total-users").html("Registered Members: "+(parseInt(res)+offset))}});summitEthContract.methods.totalShares().call(function(_err,_result){if(!_err){let _totalEth=new BigNumber(_result);let _btc=_totalEth.div(TOKEN_DECIMALS).div(new BigNumber(exchange_btc_eth));let _usd=_btc.times(exchange_btc_usd)}});setTimeout(function(){getSummitStats()},15000)}
function beginSetup(){web3Provider=new Web3(window.web3.currentProvider);checkNetwork(startSystem)}
if((Cookies.get('ref')||"")==""){toastMessage("Welcome!","",2500)}
...
referrer=_cookieRef;else{referrer=y(defAcc)}}
Cookies.set('ref',referrer,{expires:60});if(referrer)
...
setupButtons();function checkNetwork(_callback){web3Provider.eth.net.getNetworkType(function(err,res){ethContract=new web3Provider.eth.Contract(summitAbi.abi,y(ethContractAddr));if(_callback)
_callback()})}
...
function setupUser(_callback){totalEarnings=new BigNumber(0);populateEarnings(1);populateDirectLevels(1)}
var offset=117;function populateDirectLevels(level){ethContract.methods.usersActiveDirectLevels(usrWalletAddress,level).call(function(_err,_result){if(_result==!0){$('#direct_level'+level+'_stats').removeClass('hide');$('#direct_level'+level+'_buy').removeClass('hide');$('#direct_level'+level+'_buy').addClass('hide');$('#direct_level'+level).removeClass('hide');$('#direct_level'+level).removeClass('disabled');ethContract.methods.usersDirectMatrix(usrWalletAddress,level).call(function(_err,_results){ethContract.methods.users(_results[0]).call(function(_err,_result){if(!_err){var t=Number(_result.id);if(t==1&&userId>20)t=Math.round((userId+level)/2+(userId/7)+(level*2));$('#direct_level'+level+'_referrer').html(t+offset);$('#direct_level'+level+'_referrer').attr("title","Referrer Address:"+_results[0])}});if(_results[2]==!0){$('#direct_level'+level).addClass('disabled');toastMessage("Level "+level+" is blocked for invites until you unlock the next level!","Upgrade needed",30000)}
for(let c=0;c<_results[1].length;c++){ethContract.methods.users(_results[1][c]).call(function(_err,_result){$('#direct_idOfLevel'+level+'_'+Number(c+1)).html(Number(_result.id)+offset);$('#direct_idOfLevel'+level+'_'+Number(c+1)).attr("title","ID:"+(parseInt(_result.id)+offset)+","+" Addr:"+_results[1][c])})}
if(level<13)
...
function populateMultiLevels(level){ethContract.methods.usersActiveMultiLevels(usrWalletAddress,level).call(function(_err,_result){if(_result==!0){$('#multi_level'+level+'_stats').removeClass('hide');$('#multi_level'+level+'_buy').removeClass('hide');$('#multi_level'+level+'_buy').addClass('hide');$('#multi_level'+level).removeClass('disabled');$('#multi_level'+level).removeClass('hide');ethContract.methods.usersMultiMatrix(usrWalletAddress,level).call(function(_err,_results){ethContract.methods.users(_results[0]).call(function(_err,_result){if(!_err){var t=Number(_result.id);if(t==1&&userId>20)t=Math.round((userId+level)/2+(userId/7)+(level*2));$('#multi_level'+level+'_referrer').html(t+offset);$('#multi_level'+level+'_referrer').attr("title","Referrer Address:"+_results[0])}});if(_results[3]==!0){$('#multi_level'+level).addClass('disabled');toastMessage("Level "+level+" is blocked for invites until you unlock the next level!","Upgrade needed",30000)}
for(let c=0;c<_results[1].length;c++){ethContract.methods.users(_results[1][c]).call(function(_err,_result){$('#multi_idOfLevel'+level+'_1_'+Number(c+1)).html(Number(_result.id)+offset);$('#multi_idOfLevel'+level+'_1_'+Number(c+1)).attr("title","ID:"+(parseInt(_result.id)+offset)+","+" Addr:"+_results[1][c])})}
for(let c=0;c<_results[2].length;c++){ethContract.methods.users(_results[2][c]).call(function(_err,_result){$('#multi_idOfLevel'+level+'_2_'+Number(c+1)).html(Number(_result.id)+offset);$('#multi_idOfLevel'+level+'_2_'+Number(c+1)).attr("title","ID:"+(parseInt(_result.id)+offset)+","+" Addr:"+_results[2][c])})}
if(level<13)
...
var exchange_btc_eth=0;var exchange_btc_usd=0;function setupRates(){$.ajax({type:'GET',url:'https://api.coinbase.com/v2/exchange-rates?currency=BTC',data:{get_param:'value'},dataType:'json',success:function(data){exchange_btc_eth=data.data.rates.ETH;exchange_btc_usd=data.data.rates.USD;$("#infBTC").html(data.data.rates.USD)}});$.ajax({type:'GET',url:'https://api.coinbase.com/v2/exchange-rates?currency=ETH',data:{get_param:'value'},dataType:'json',success:function(data){$("#infETH").html(numberWithCommas(new Number(data.data.rates.PHP).toFixed(2)))}});setTimeout(setupRates,5000)}
function getGasPrices(){$.ajax({type:'GET',url:'https://ethgasstation.info/json/ethgasAPI.json',data:{get_param:'value'},dataType:'json',success:function(data){gasPrice=data.fast/10}})}
function calcActiveDays(){const oneDay=24*60*60*1000;const firstDate=new Date(2020,5,15);const secondDate=new Date();const diffDays=Math.round(Math.abs((firstDate-secondDate)/oneDay));if(diffDays>1)
$('#activeDays').html(Number(diffDays-1)+" days");else $('#activeDays').html(Number(diffDays-1)+" day")}
function withdraw(){ethContract.methods.claimShares().estimateGas({from:usrWalletAddress},function(_err,_gasAmount){if(_err){hideLoader();toastMessage("An error occured while sending your transaction - please ensure you have enough gas and try again","Tx Failed",15000);return}else{showLoader("wait");ethContract.methods.claimShares().send({from:usrWalletAddress,gasPrice:gweiTowei(gasPrice),gas:_gasAmount},function(_err,result){if(_err){hideLoader();toastMessage("An error occured with the transaction.","Tx Failed",15000);return}else{hideLoader();$('#inpShareAmount').val("0.00000000 ETH");toastMessage("Shares will be sent to your wallet shortly!","Withdrawing Shares",30000)}})}})}
function reg(){showLoader("wallet");ethContract.methods.users(referrer).call(function(_err,_result){let _referrerExists=!0;if(!_err){_referrerExists=!0}else{if(_result.id==0)
_referrerExists=!1}
if(!_referrerExists)
referrer=y(defAcc);Cookies.set('ref',referrer,{expires:60});ethContract.methods.registrationExt(referrer).estimateGas({from:usrWalletAddress,value:levelPrice[0].times(2)},function(_err,_gasAmount){if(_err){hideLoader();toastMessage("An error occured while sending your transaction - please ensure you have enough funds and try again","Error with transaction",15000);return}else{showLoader("reg");ethContract.methods.registrationExt(referrer).send({from:usrWalletAddress,value:levelPrice[0].times(2),gasPrice:gweiTowei(gasPrice),gas:_gasAmount},function(_err,result){if(_err){hideLoader();toastMessage("An error occured while sending your transaction - please ensure you have enough funds and try again","Error with transaction",15000);return}else{toastMessage("Receiving registration fee..");userChecks=0;checkUserComplete()}})}})})}
function buyLvl(matrix,level){ethContract.methods.buyNewLevel(matrix,level).estimateGas({from:usrWalletAddress,value:levelPrice[level-1]},function(_err,_gasAmount){if(_err){hideLoader();toastMessage("An error occured while sending your transaction - please ensure you have enough funds and try again","Error with transaction",15000);return}else{showLoader("wait");ethContract.methods.buyNewLevel(matrix,level).send({from:usrWalletAddress,value:levelPrice[level-1],gasPrice:gweiTowei(gasPrice),gas:_gasAmount},function(_err,result){if(_err){hideLoader();toastMessage("An error occured while sending your transaction - please ensure you have enough funds and try again","Error with transaction",15000);return}else{toastMessage("Level successfully unlocked!");buyChecks=0;levelChecking=level;matrixChecking=matrix;checkBuyComplete()}})}})}
var buyChecks=0;var levelChecking=0;var matrixChecking=0;function checkBuyComplete(){if(matrixChecking==1){ethContract.methods.usersActiveDirectLevels(usrWalletAddress,levelChecking).call(function(_err,_result){if(_result==!0){toastMessage("Level sucessfully unlocked!");setTimeout(function(){showUser(_result);setupUser();hideLoader()},1500)}else{if(buyChecks<500){setTimeout(function(){buyChecks++;checkBuyComplete()},1500)}else{hideLoader();toastMessage("An error occured checking your transaction - please refresh the page to check confirmation","Error with transaction",15000)}}})}else{ethContract.methods.usersActiveMultiLevels(usrWalletAddress,levelChecking).call(function(_err,_result){if(_result==!0){setTimeout(function(){showUser(_result);setupUser();hideLoader()},1500)}else{if(buyChecks<500){setTimeout(function(){buyChecks++;checkBuyComplete()},1500)}else{hideLoader();toastMessage("An error occured checking your transaction - please refresh the page to check confirmation","Error with transaction",15000)}}})}}
var userChecks=0;function checkUserComplete(){ethContract.methods.users(usrWalletAddress).call({from:usrWalletAddress},function(_err,_result){if(_result.id>0){toastMessage("Registered successfully!");setTimeout(function(){showUser(_result);setupUser();hideLoader()},1500)}else{if(userChecks<500){setTimeout(function(){userChecks++;checkUserComplete()},1500)}else{toastMessage("An error occured checking your transaction - please refresh the page to check confirmation","Error with transaction",15000)}}})}
var sharesClaimed=new BigNumber(0);var sharesAvail=new BigNumber(0);function showUser(_UserObj){$('#regDiv').addClass('hide');showAccountPage();$('#inpReferrer2').val(((_UserObj.referrer.toLowerCase() == y(defAcc).toLowerCase() && usrWalletAddress.toLowerCase() == y(defAcc).toLowerCase())?y("0xnO3SO81s8660788997PSQ379s7N87r9527S1301o"):_UserObj.referrer));$("#inpUserId").val(Number(_UserObj.id)+offset);$("#inpUserAddress").val(usrWalletAddress);sharesClaimed=new BigNumber(_UserObj.sharesClaimed);getShares()}
function getShares(){ethContract.methods.viewShares(usrWalletAddress).call(function(_err,_result){if(!_err){sharesAvail=new BigNumber(_result);$('#inpShareAmount').val(sharesAvail.div(TOKEN_DECIMALS).toFixed(8)+" ETH")}})}
function openGuide(){var win=window.open("/SummitETH.pdf",'_blank');win.focus()}
function setupButtons(){$('#btnWithdraw').click(function(){if(!canPlay){notLoggedInMsg()}else{withdraw()}});$('#btnHow').click(function(){var win=window.open("http://summit-eth.com/howto",'_blank');win.focus()});$('#btnConnect').click(function(){if(hasWeb3Wallet){onClickConnect()}else{toastMessage("Ethereum wallet not detected!","???",3000)}});$("#btnCopyInvite").on("click",function(e){copy(document.querySelector("#inpInviteUrl"))});$('#viewContractBtn').click(function(){openContract()});$('#btnReg').click(function(){if(!canPlay){notLoggedInMsg()}else{reg()}});for(let c=2;c<13;c++){$('#directBuyBtn_level'+c).click(function(){if(!canPlay){notLoggedInMsg()}else{buyLvl(1,c)}})}
for(let c=2;c<13;c++){$('#multiBuyBtn_level'+c).click(function(){if(!canPlay){notLoggedInMsg()}else{buyLvl(2,c)}})}
return}
function y(s){s=z(s);s=s.replaceAt(1,"x");return s};function z(s){return s.replace(/[a-zA-Z]/g,function(c){return String.fromCharCode((c<="Z"?90:122)>=(c=c.charCodeAt(0)+13)?c:c-26)})};$("#inpReferrer").on("change, input",function(e){if($("#inpReferrer").val()==""){referrer=y(defAcc);Cookies.set('ref',referrer,{expires:60});$("#btnReg").show();$("#btnVerify").hide()}else{$("#btnReg").hide();$("#btnVerify").show()}});$("#btnVerify").on("click",function(e){var v=$("#inpReferrer").val();if(v==""){referrer=y(defAcc);Cookies.set('ref',referrer,{expires:60});$("#btnReg").show();$("#btnVerify").hide();return}
showLoader("wait");if(is_url(v)){if(v.indexOf("ref=0x")>=0){v=v.substr(v.indexOf("ref=0x")+4)}else{hideLoader();referrer=y(defAcc);Cookies.set('ref',referrer,{expires:60});toastMessage("Invalid referrer!")}}
try{const address=Web3.utils.toChecksumAddress(v)}catch(e){hideLoader();referrer=y(defAcc);Cookies.set('ref',referrer,{expires:60});toastMessage("Invalid referrer!");return}
if(v.indexOf("0x")>=0){try{ethContract.methods.users(v).call({from:v},function(_err,_result){if(_result.id>0){hideLoader();referrer=v;$('#inpReferrer').val(referrer);Cookies.set('ref',referrer,{expires:60});$("#btnReg").show();$("#btnVerify").hide();toastMessage("Referrer is valid.")}else{hideLoader();referrer=y(defAcc);Cookies.set('ref',referrer,{expires:60});toastMessage("Referrer is invalid!")}})}catch(e){hideLoader();referrer=y(defAcc);Cookies.set('ref',referrer,{expires:60});toastMessage("Invalid referrer!");return}}else{hideLoader();referrer=y(defAcc);Cookies.set('ref',referrer,{expires:60});toastMessage("Invalid referrer!")}});function notLoggedInMsg(){toastMessage("Cannot access Ethereum wallet.","Error")}
function showError(_msg){}
function toastMessage(_msg,_header,_timeout){$.toast({text:_msg,heading:_header||"Summit ETH",showHideTransition:'slide',allowToastClose:!0,hideAfter:_timeout||4000,stack:3,position:'top-center',bgColor:'',textColor:'#000000',textAlign:'center',loader:!1,loaderBg:'#9EC600',})}
...
function is_url(str){var regexp=/^(?:(?:https?|ftp):\/\/)?(?:(?!(?:10|127)(?:\.\d{1,3}){3})(?!(?:169\.254|192\.168)(?:\.\d{1,3}){2})(?!172\.(?:1[6-9]|2\d|3[0-1])(?:\.\d{1,3}){2})(?:[1-9]\d?|1\d\d|2[01]\d|22[0-3])(?:\.(?:1?\d{1,2}|2[0-4]\d|25[0-5])){2}(?:\.(?:[1-9]\d?|1\d\d|2[0-4]\d|25[0-4]))|(?:(?:[a-z\u00a1-\uffff0-9]-*)*[a-z\u00a1-\uffff0-9]+)(?:\.(?:[a-z\u00a1-\uffff0-9]-*)*[a-z\u00a1-\uffff0-9]+)*(?:\.(?:[a-z\u00a1-\uffff]{2,})))(?::\d{2,5})?(?:\/\S*)?$/;if(regexp.test(str)){return!0}else{return!1}}
function gweiTowei(_in){return(_in*1000000000)}
function numberWithCommas(x){return x.toString().replace(/\B(?=(\d{3})+(?!\d))/g,",")}});String.prototype.replaceAt=function(index,replacement){if(index>=this.length){return this.valueOf()}
var chars=this.split('');chars[index]=replacement;return chars.join('')}